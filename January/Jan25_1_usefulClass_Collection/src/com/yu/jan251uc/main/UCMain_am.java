		...
		try {
			...
			while (type != XmlPullParser.END_DOCUMENT) {
				if (type == XmlPullParser.START_TAG) {
					t = xpp.getName();
					if (t.equals("data")) {
						wis.add(new WeatherInfo());
					}
				} else if (type == XmlPullParser.TEXT) {
					if (t.equals("hour")) {
						wis.get(wis.size() - 1).setHour(xpp.getText());
					} else if (t.equals("day")) {
						wis.get(wis.size() - 1).setDate(xpp.getText());
					} else if (t.equals("temp")) {
						wis.get(wis.size() - 1).setTemp(Double.parseDouble(xpp.getText()));
					} else if (t.equals("wfKor")) {
						wis.get(wis.size() - 1).setWfKor(xpp.getText());
					}
